...
describe('validations', () => {
  it('single validation rule - or', () => {
    const validationRule: ValidationRule = DOOV.when(B.and(ID.greaterOrEquals(3).or(NAME.endsWith('st')))).validate();
  ...
  it('single validation rule - and', () => {
    const validationRule: ValidationRule = DOOV.when(B.and(ID.greaterOrEquals(3).and(NAME.endsWith('st')))).validate();
    const context = new DefaultContext(false);
    const result = validationRule.execute(model, context);
  ...
  it('single validation rule - matchAny', () => {
    const validationRule: ValidationRule = DOOV.when(
      DOOV.matchAny(
        ID.greaterOrEquals(3).and(NAME.endsWith('st')),
        ID.greaterOrEquals(0).and(NAME.endsWith('st')),
        ID.greaterOrEquals(0).and(NAME.endsWith('ts'))
      )
    ).validate();
    const context = new DefaultContext(false);
    const result = validationRule.execute(model, context);
  ...
  it('should be triggered on first rule', () => {
    const validationRule: ValidationRule = DOOV.validations(
  ...
  it('should be triggered on second rule', () => {
    const validationRule: ValidationRule = DOOV.validations(
      DOOV.when(B.and(ID.greaterOrEquals(0))).validate(),
